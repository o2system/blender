...
class Manifest {
     ...
     * @param {string} name
     */
    constructor(name = 'blender-manifest.json') {
        this.manifest = {};
        this.name = name;
    }
    ...
    /**
     * Refresh the blender-manifest.js file.
     */
    refresh() {
        File.find(this.path())
            .makeDirectories()
    ...
    flattenAssets(stats) {
        ...
        // If there's a temporary blender.js chunk, we can safely remove it.
        if (assets.blender) {
            assets.blender = without(assets.blender, 'blender.js');
        }
