...
class Vue {
    ...
    updateCssLoaders(webpackConfig) {
        ...
        let sassCallback = rule => {
            if (Blender.seesNpmPackage('sass')) {
                rule.loaders.find(
            ...
            if (Config.globalVueStyles) {
                rule.loaders.push({
                    ...
                    options: {
                        resources: Blender.paths.root(Config.globalVueStyles)
                    }
                });
            }
        };
     ...
     */
    postCssOptions() {
        if (Blender.components.get('postCss')) {
            return {
                plugins: Blender.components.get('postCss').details[0].postCssPlugins
            };
        }
        ...
        if (File.exists(Blender.paths.root('postcss.config.js'))) {
            return {};
        }
    ...
    extractPlugin() {
        ...
        if (typeof Config.extractVueStyles === 'boolean') {
            let preprocessorName = Object.keys(Blender.components.all())
                .reverse()
                .find(componentName => {
